from tkinter import *
import tkinter as tk
from tkinter import ttk
from PIL import Image, ImageTk

# Main window
root = Tk()
root.title("BMI calculator")
root.geometry("470x580+300+200")
root.resizable(False, False)
root.configure(bg="#f0f1f5")

def BMI():
    h = float(Height.get())
    w = float(Weight.get())

    #convert height into meters
    m = h / 100
    bmi =round(float( w / m**2 ))
    label1.config(text=bmi)


    if bmi <=18.5:
        label2.config(text = "UnderWeight !")
        label3.config(text =" Your weight is less than the \nweight of normal body.")
    elif bmi > 18.5 and bmi <= 25:
        label2.config(text = "Normal")
        label3.config(text = "You're Healthy !")
    elif bmi>25 and bmi<=30:
        label2.config(text = "OverWeight !")
        label3.config(text = "You're slightly overweighted")
    else:
        label2.config(text = "Obes!")
        label3.config(text = "Warning ! \nYou have to reduce your weight.")


# Icon
icon_image = Image.open("images/icon.png")
image_icon = ImageTk.PhotoImage(icon_image)
root.iconphoto(False, image_icon)

#top
top_image = Image.open("images/top.jpg")
image_top = ImageTk.PhotoImage(top_image)
top_image = Label(root, image = image_top, background="white")
top_image.place(x= -10, y = -10)

#bottom
Label(root, width = 72, height = 18, bg = "white").pack(side = BOTTOM)

#two boxes
box_image = Image.open("images/box.jpg")
box = ImageTk.PhotoImage(box_image)
Label(root, image = box).place(x= 20, y = 100)
Label(root, image = box).place(x= 240, y = 100)

#scale
Scale_image = Image.open("images/scale.png")
scale = ImageTk.PhotoImage(Scale_image)
Label(root, image = scale, bg = "white").place(x = 20, y = 310)

#<-------------------Slider1----------------------->
current_value = tk.DoubleVar()

def get_current_value():
    return '{: .2f}'.format(current_value.get())

def slider_changed(event):
    Height.set(get_current_value())

    size= int(float(get_current_value()))
    img   = (Image.open("images/man.jpg"))
    resized_image = img.resize((50, 10+size))
    photo2 = ImageTk.PhotoImage(resized_image)
    secondimage.config(image=photo2)
    secondimage.place(x = 70, y = 550 - size)
    secondimage.image = photo2

#Command to change background color of scale
style = ttk.Style()
style.configure("TScale", background = "white")


slider = ttk.Scale(root, from_= 0 , to = 220, orient = 'horizontal',command=slider_changed,  variable=current_value)
slider.place(x = 80, y = 250)

#<----------------------END(SLIDER1)----------------------->


#<----------------------Slider2------------------------------------->
current_value2 = tk.DoubleVar()

def get_current_value2():
    return '{: .2f}'.format(current_value2.get())

def slider_changed2(event):
    Weight.set(get_current_value2())

#Command to change background color of scale
style2 = ttk.Style()
style2.configure("TScale", background = "white")


slider2 = ttk.Scale(root, from_= 0 , to = 200, orient = 'horizontal',command=slider_changed2,  variable=current_value2)
slider2.place(x = 290, y = 250)

#<----------------------END(SLIDER2)----------------------->


#Entry box
Height = StringVar()
Weight = StringVar()

Height.set(get_current_value())
height = Entry(root, textvariable=Height, width=5, font="arial 50", bg="#fff", fg = "#000", bd = 0, justify=CENTER) #Align text in center
height.place(x = 28, y = 160)

Weight.set(get_current_value2())
weight = Entry(root, textvariable=Weight, width=5, font="arial 50", bg="#fff", fg = "#000", bd = 0, justify=CENTER) #Align text in center
weight.place(x = 255, y = 160)

#Man image
secondimage = Label(root, bg = "white")
secondimage.place(x = 70, y = 530)


Button(root, text = "view Report", width = 15, height = 2, font = "arial 10 bold", bg = "#1f6e68", fg = "white", command= BMI).place(x = 280, y = 340)

label1 = Label(root, font = "arial 60 bold", bg = "white", fg="black")
label1.place(x = 130, y = 305)

label2 = Label(root, font = "arial 20 bold", bg = "white", fg="#3b3a3a")
label2.place(x = 240, y = 430)

label3 = Label(root, font = "arial 10 bold", bg = "white")
label3.place(x = 200, y = 500)




# Run the application
root.mainloop()
